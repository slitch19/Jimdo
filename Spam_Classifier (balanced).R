...
# country : Create ""others" category for country 
...
# language:  Create ""others" category for language
...
# Extract 95% of the rows as the train set leaving the 5% as the test set
# This is set high since we still have to delete a number of rows later on to create a balance class labels in the train set..
set.seed(10)
train_index =  createDataPartition(final_data$class, p = 0.95, 
                                  list = FALSE, 
                                  times = 1)
train_set = final_data[train_index,]
test_set = final_data[-train_index,]

# Sample from the majority class ("ham") such that the distribition between spam and ham on the train set is 50/50.
set.seed(11)
spam_rows = train_set[which(train_set$class == "spam"),]          # get all rows with class "spam" from the train set
ham_rows  = sample_n(train_set[which(train_set$class == "ham"),], 
                     size = nrow(spam_rows))                      # sample same number rows but with class "ham" from the train set
train_set = as.data.frame(rbind(ham_rows, spam_rows))             # combine the two to generate a balanced data frame

# Let's check if the distribution between spam and ham is preserved in the test set.
table(final_data$class)/nrow(final_data)
table(train_set$class)/nrow(train_set)      # now we achieved a balanced data set to build our model
table(test_set$class)/nrow(test_set)        # test set still have the ditribution of the orginal data.

nrow(test_set) /(nrow(test_set) + nrow(train_set))   # test set is 23% of the total data we wil be using to build and test the model
...
set.seed(12)
rf1_model = randomForest(class~. -email_main_provider_grouped -country_grouped, data = train_set, ntree = 500)
...
rf1_conf_matrix       #      prediction_class
                      ...
                      #  ham   6713 2126
                      #  spam   151  691
rf1_accuracy          #  0.764797
rf1_precision         #  0.2452964
rf1_recall            #  0.8206651
rf1_auc               #  0.8747445
...
set.seed(13)
rf_importance = randomForest(class~., data = train_set, ntree = 500, importance = T)
...
#             then select the optimal model as final random forest model (smaller number of predictors yet good AUC value)
...
# Let's split further our train set into small_train_set and validation_set for model selection.
set.seed(14)
small_train_index = createDataPartition(train_set$class, p = 0.90, list = FALSE, times = 1)

small_train_set = train_set[small_train_index,]   # 90% small_train_set
validation_set = train_set[-small_train_index,]   # 10% validation_set
...
for (i in 1 : length(sorted)){
  rf2_model[[i]] = randomForest(class~., data = small_train_set[,c("class", sorted[1:i])], ntree = 500) 
...
final_predictors = sorted[1:6]     # based on the plot We consider up to the top 6 important predictors

# final predictors for our final model
#  "email_main_provider_grouped" 
#  "country_grouped"             
# "language_grouped"           
#  "is_visible"                 
...
# Final Model
set.seed(16)
final_rf_model = randomForest(class~., data = train_set[,c("class", final_predictors)], ntree = 1000)
...
final_rf_conf_matrix #      prediction_class
                     ...
                     #       ham  7492 1347
                     #       spam  98  744
final_rf_accuracy    #  0.8507386
final_rf_precision   #  0.3558106
final_rf_recall      #  0.8836105
final_rf_auc         #  0.9369616
...
adaboost1_accuracy    # 0.8464002
adaboost1_precision   # 0.3366059
adaboost1_recall      # 0.8551069
adaboost1_auc         # 0.5956658
...
adaboost2_accuracy    # 0.8479496
adaboost2_precision   # 0.3442136
adaboost2_recall      # 0.8266033
adaboost2_auc         # 0.7391765
